...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'ticket_count' in counters:
            values['ticket_count'] = (
                request.env['helpdesk_ticket'].search_count(self._prepare_helpdesk_tickets_domain())
                if request.env['helpdesk_ticket'].check_access_rights('read', raise_exception=False)
                else 0
            )
        return values
    ...
    def my_helpdesk_tickets(self, page=1, date_begin=None, date_end=None, sortby=None, filterby='all', search=None, groupby='none', search_in='content', **kw):
        ...
        if filterby in ['last_message_sup', 'last_message_cust']:
            ...
            messages = request.env['mail.message'].search_read([('model', '=', 'helpdesk_ticket'), ('subtype_id', '=', discussion_subtype_id)], fields=['res_id', 'author_id'], order='date desc')
            last_author_dict = {}
            for message in messages:
                if message['res_id'] not in last_author_dict:
                    last_author_dict[message['res_id']] = message['author_id'][0]

            ticket_author_list = request.env['helpdesk_ticket'].search_read(fields=['id', 'partner_id'])
